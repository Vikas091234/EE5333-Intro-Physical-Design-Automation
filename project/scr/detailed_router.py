...
New fixes in this revision:
  FIX-OBST-1     ObstStore.add() used list.append() + a one-time build() sort.
                 Any add() after build() broke the binary-search invariant.
                 Replaced with bisect.insort so the list is always sorted;
                 build() is now a no-op kept for API compatibility.

  FIX-ROUTEDOBST Committed wire segments were only in OccupancyMap (soft
                 penalty), NOT in ObstStore (hard obstacle check via
                 obst.blocks()).  The jog-selector therefore had no hard
                 knowledge of previously routed wires and could place new
                 wires directly on top of them.
                 Fixed: after each net's rects are clamped and written to DEF,
                 they are also added to ObstStore.  FIX-OBST-1 keeps the list
                 sorted without a separate build() call.

  FIX-PREEXIST   Wires already present in the input DEF (e.g. pre-routed clock
                 trees placed by CTS) were never registered in OccupancyMap.
                 The router treated those tracks as empty and routed other nets
                 onto the same met1 rows, causing widespread same-track DRC
                 violations (e.g. clknet_1_1__leaf_clk Y=31380 conflicting
                 with 7 other nets).
                 Fixed: before routing any net, all existing wire rects in the
                 DEF are walked and registered in both occ and obst so the
                 router avoids them.

  NOTE:  Cell PIN shapes (PORT sections) are intentionally NOT added to
         ObstStore.  Adding them massively increases obstruction density,
         leaves the router with fewer track choices, and forces unrelated nets
         onto already-congested rows — producing more violations, not fewer.
         The AP builders already ensure stubs cover the pin rect; OccupancyMap
         handles same-track avoidance as a penalty.
"""
...
class ObstStore:
    ...
    def add(self, layer, x1, y1, x2, y2):
        # FIX-OBST-1: Use bisect.insort instead of append so the list stays
        # sorted at all times.  Routed wires are now added AFTER build() via
        # FIX-ROUTEDOBST, so we can no longer rely on a single up-front sort.
        bisect.insort(self._rects[layer],
                      (min(x1,x2), min(y1,y2), max(x1,x2), max(y1,y2)))

    def build(self):
        # FIX-OBST-1: No-op — list is always sorted by add().
        # Kept for API compatibility with the call in detailed_route().
        pass
...
def detailed_route(input_def, input_lef, input_guide, output_def):
    ...
    # FIX-PREEXIST: Register all wire rects that are ALREADY in the input DEF
    # (e.g. pre-routed clock trees placed by CTS) into occ and obst BEFORE
    # routing begins.  Without this, the router treats those tracks as free and
    # routes other nets onto the same rows, producing massive same-track DRC
    # violations (e.g. the Y=31380 clknet_1_1__leaf_clk row).
    occ  = OccupancyMap()
    print("[DR] Registering pre-existing DEF wires ...")
    for net in deff.nets():
        nname = net.name()
        try:
            for (layer, x1, y1, x2, y2) in net.rects():
                if layer not in LAYER_IDX: continue
                register_rect_occ(occ, layer, x1, y1, x2, y2, nname)
                obst.add(layer, x1, y1, x2, y2)
        except (AttributeError, TypeError):
            pass   # API may not expose pre-existing rects; silently skip
    ...
    for net_name, net in ordered_nets:
        ...
        for (layer,x1,y1,x2,y2) in all_rects:
            ...
            # FIX-ROUTEDOBST: Add every committed wire to ObstStore so
            # subsequent nets treat it as a hard obstacle in obst.blocks().
            # FIX-OBST-1 keeps the list sorted without a rebuild() call.
            obst.add(layer, rx1, ry1, rx2, ry2)
